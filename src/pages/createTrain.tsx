...
export default function App() {
  ...
  // 2. Função para adicionar o exercício escolhido à lista da ficha
  function handleAddExercicio() {
    if (!idSelecionado) return alert("Selecione um exercício primeiro");
  ...
  async function handleSaveFicha() {
    if (!nomeTreino || exerciciosTreino.length === 0)
      return alert("Dê um nome ao treino e adicione pelo menos um exercício");

    try {
      // Enviamos a lista que montamos
  ...
  return (
    <div className="bg-zinc-950 min-h-screen text-white pb-10">
      ...
      <div className="flex flex-col px-6 mt-6 space-y-4 max-w-xl mx-6 bg-[#0f0f0f] border border-white/10 rounded-xl p-6">
          ...
          {categorias.map((cat) => (
            <button
              ...
              onClick={() => {
                ...
                setIdSelecionado(""); // Limpa o select ao trocar de categoria
              }}
        ...
        <div className="flex gap-2">
          <select
            className="flex-1 bg-zinc-900 text-gray-300 border border-white/10 focus:border-[#22c55e] focus:outline-none px-3 py-2 rounded"
            value={idSelecionado}
      ...
      {/* Lista visual dos exercícios que estão sendo adicionados */}
